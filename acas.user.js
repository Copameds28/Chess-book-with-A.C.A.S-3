...
// ─────────────────────────────────────────────────────────────────────────────
const OPENING_BOOK_URL = 'https://raw.githubusercontent.com/Copameds28/Chess-book-with-A.C.A.S-3/main/gm_openings.json'; // ← paste your raw GitHub URL here
...
let openingBookArrowElem = null;
let openingBookEnabled = true; // can be toggled via GM menu
...
function clearOpeningBookArrow() {
    if(openingBookArrowElem) {
        try { openingBookArrowElem.remove(); } catch(_) {}
        openingBookArrowElem = null;
    }
}
...
function lookupAndDrawOpeningBookArrow(fen) {
    ...
    // Key on position + side-to-move only (ignore castling rights / clocks)
    const fenParts = fen.split(' ');
    const fenKey = fenParts.slice(0, 2).join(' ');

    const moves = openingBook[fenKey];
    if(!moves || moves.length === 0) return; // out of book — no arrow shown

    // Pick highest-weight book move
    const best = moves.reduce((a, b) => (b.weight > a.weight ? b : a));
    const from = best.uci.slice(0, 2);
    const to   = best.uci.slice(2, 4);

    // Draw a distinct gold arrow for the book move
    openingBookArrowElem = BoardDrawer.createShape('arrow', [from, to], {
        style: [
            'fill: #f0a500;',
            'opacity: 0.88;',
            'stroke: rgb(0 0 0 / 45%);',
...
// Allow toggling book arrows from the Greasemonkey/Tampermonkey menu
