...
function showWinRateTooltip(moves, totalWeight) {
    ...
    // Default position: right of board, or last saved position
    const boardElem = getBoardElem();
    ...
    tooltip.innerHTML =
        `<div style="font-size:10px;color:#666;margin-bottom:7px;text-transform:uppercase;letter-spacing:0.6px;cursor:move;user-select:none" id="acas-winrate-handle">&#9776; GM Book moves</div>` +
        rows;
    ...
    // Drag logic — only drag when grabbing the handle bar
    ...
    if(handle) {
        ...
        handle.addEventListener('mousedown', e => {
            ...
            startX = e.clientX;
            startY = e.clientY;
            origLeft = parseInt(tooltip.style.left);
            origTop  = parseInt(tooltip.style.top);
            e.preventDefault();
        });
        ...
        document.addEventListener('mouseup', () => {
            if(!dragging) return;
            dragging = false;
            // Save position so it persists across moves
            openingBookTooltipPos = {
...
function lookupAndDrawOpeningBookArrow(fen) {
    ...
    toShow.forEach((move, rank) => {
        ...
        const elem = BoardDrawer.createShape('arrow', [from, to], {
            ...
            lineWidth: lineWidth, arrowheadWidth: arrowheadWidth, arrowheadHeight: arrowheadHeight, startOffset: 26
        });
...
GM_registerMenuCommand('[ACAS] Toggle Opening Book Arrows', () => {
    ...
    GM_notification({ title: 'ACAS Opening Book', text: `Opening book arrows ${openingBookEnabled ? 'enabled \u2713' : 'disabled \u2717'}`, timeout: 2500 });
});
