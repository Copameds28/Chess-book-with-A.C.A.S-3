...
// When the current position is in the book, a gold arrow is drawn for the
// highest-weight book move. When the position falls out of the book the arrow
// disappears automatically.
//
// Default book is a pre-converted subset of GM-level Polyglot lines hosted on
// GitHub. You can swap OPENING_BOOK_URL to point at any compatible JSON book.
// Format: { "<pos> <color>": [{ "uci": "e2e4", "weight": 100 }, …], … }
// ─────────────────────────────────────────────────────────────────────────────
const OPENING_BOOK_URL = 'https://cdn.jsdelivr.net/gh/Copameds28/Chess-book-with-A.C.A.S-3@main/gm_openings.json'; // ← paste your raw GitHub URL here

// Fallback built-in mini-book (most common GM openings) used when the remote
// book fails to load, so the feature still works offline.
const BUILTIN_OPENING_BOOK = {
    "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w": [
        { uci: "e2e4", weight: 100 }, { uci: "d2d4", weight: 90 },
        { uci: "c2c4", weight: 70 }, { uci: "g1f3", weight: 60 }
    ],
    "rnbqkbnr/pppppppp/8/8/4P3/8/PPPP1PPP/RNBQKBNR b": [
        { uci: "e7e5", weight: 100 }, { uci: "c7c5", weight: 95 },
        { uci: "e7e6", weight: 80 }, { uci: "c7c6", weight: 75 }
    ],
    "rnbqkbnr/pppppppp/8/8/3P4/8/PPP1PPPP/RNBQKBNR b": [
        { uci: "d7d5", weight: 100 }, { uci: "g8f6", weight: 90 },
        { uci: "e7e6", weight: 80 }, { uci: "c7c5", weight: 75 }
    ],
    "rnbqkbnr/pppp1ppp/8/4p3/4P3/8/PPPP1PPP/RNBQKBNR w": [
        { uci: "g1f3", weight: 100 }, { uci: "f1c4", weight: 80 },
        { uci: "f2f4", weight: 60 }, { uci: "d2d4", weight: 55 }
    ],
    "rnbqkbnr/pp1ppppp/8/2p5/4P3/8/PPPP1PPP/RNBQKBNR w": [
        { uci: "g1f3", weight: 100 }, { uci: "c2c3", weight: 70 },
        { uci: "f1c4", weight: 60 }, { uci: "b1c3", weight: 55 }
    ],
    "rnbqkbnr/pppp1ppp/4p3/8/4P3/8/PPPP1PPP/RNBQKBNR w": [
        { uci: "d2d4", weight: 100 }, { uci: "g1f3", weight: 80 },
        { uci: "b1c3", weight: 60 }
    ],
    "rnbqkbnr/pppppppp/8/8/2P5/8/PP1PPPPP/RNBQKBNR b": [
        { uci: "e7e5", weight: 90 }, { uci: "c7c5", weight: 100 },
        { uci: "g8f6", weight: 75 }, { uci: "e7e6", weight: 70 }
    ],
    "rnbqkb1r/pppppppp/5n2/8/3P4/8/PPP1PPPP/RNBQKBNR w": [
        { uci: "c2c4", weight: 100 }, { uci: "g1f3", weight: 90 },
        { uci: "e2e3", weight: 60 }
    ],
    "rnbqkbnr/ppp1pppp/8/3p4/3P4/8/PPP1PPPP/RNBQKBNR w": [
        { uci: "c2c4", weight: 100 }, { uci: "g1f3", weight: 90 },
        { uci: "e2e3", weight: 50 }, { uci: "b1c3", weight: 70 }
    ]
};
...
let wasInBook = false;
let openingBookOutIndicatorElem = null;

async function loadOpeningBook() {
    try {
        const res = await fetch(OPENING_BOOK_URL, { cache: 'force-cache' });
        if(!res.ok) throw new Error(`HTTP ${res.status}`);
        openingBook = await res.json();
        if(debugModeActivated) console.log('[ACAS Opening Book] Remote book loaded:', Object.keys(openingBook).length, 'positions');
    } catch(e) {
        if(debugModeActivated) console.warn('[ACAS Opening Book] Remote load failed, using built-in mini-book:', e.message);
        openingBook = BUILTIN_OPENING_BOOK;
    }
}

function clearOpeningBookArrow() {
    for(const elem of openingBookArrowElems) {
        try { elem.remove(); } catch(_) {}
    }
    openingBookArrowElems = [];
}

function clearOutOfBookIndicator() {
    if(openingBookOutIndicatorElem) {
        try { openingBookOutIndicatorElem.remove(); } catch(_) {}
        openingBookOutIndicatorElem = null;
    }
}

function showOutOfBookIndicator() {
    clearOutOfBookIndicator();

    // Use ACAS's own getBoardElem() so we always find the right element
    const boardElem = getBoardElem();
    if(!boardElem) return;
    ...
    div.style.cssText = [
        ...
        'border:3px solid rgba(220,50,50,0.75)',
        'border-radius:4px',
        'pointer-events:none',
        'z-index:99999',
        'opacity:1',
    ...
    openingBookOutIndicatorElem = div;

    // Start fade after 500ms, remove after 1.3s
    setTimeout(() => { if(div.parentNode) div.style.opacity = '0'; }, 500);
    setTimeout(() => { clearOutOfBookIndicator(); }, 1300);
}
...
function showWinRateTooltip(moves, totalWeight) {
    ...
    const rankColors = ['#f0a500', '#aaaaaa', '#cd7f32'];
    ...
    const rows = topMoves.map((m, i) => {
        ...
        const color = rankColors[i] || '#888888';
        return `<div style="margin-bottom:6px">` +
            ...
            `</div>` +
            `<div style="background:#444;border-radius:3px;height:5px">` +
            `<div style="background:${color};height:5px;border-radius:3px;width:${barWidth}%"></div>` +
            `</div>` +
            `</div>`;
    }).join('');
    ...
    tooltip.innerHTML =
        `<div style="font-size:10px;color:#888;margin-bottom:7px;text-transform:uppercase;letter-spacing:0.6px">GM Book moves</div>` +
        rows;
...
function lookupAndDrawOpeningBookArrow(fen) {
    ...
    const fenKey = fenParts.slice(0, 2).join(' ');

    const moves = openingBook[fenKey];
    const nowInBook = !!(moves && moves.length > 0);

    // Flash red border when transitioning out of book
    if(wasInBook && !nowInBook) {
        showOutOfBookIndicator();
    }
    wasInBook = nowInBook;
    ...
    const sorted = [...moves].sort((a, b) => b.weight - a.weight);
    const toShow = sorted.slice(0, openingBookMaxMoves);
    const maxWeight = toShow[0].weight;

    toShow.forEach((move, rank) => {
        const from = move.uci.slice(0, 2);
        const to   = move.uci.slice(2, 4);
        const opacity = Math.max(0.35, 0.88 - rank * (0.45 / Math.max(toShow.length - 1, 1)));
        const weightRatio = move.weight / maxWeight;
        const lineWidth      = Math.round(12 + weightRatio * 16);
        const arrowheadWidth = Math.round(40 + weightRatio * 32);
        const arrowheadHeight= Math.round(30 + weightRatio * 24);

        const elem = BoardDrawer.createShape('arrow', [from, to], {
            style: [
                `fill:${openingBookPrimaryColor};`,
        ...
        if(elem) openingBookArrowElems.push(elem);
    });
...
GM_registerMenuCommand('[ACAS] Toggle Opening Book Arrows', () => {
    openingBookEnabled = !openingBookEnabled;
    if(!openingBookEnabled) { clearOpeningBookArrow(); hideWinRateTooltip(); }
    GM_notification({
        title: 'ACAS Opening Book',
        text: `Opening book arrows ${openingBookEnabled ? 'enabled \u2713' : 'disabled \u2717'}`,
        timeout: 2500
    });
});

// Set how many book moves to show (1-5)
GM_registerMenuCommand('[ACAS] Set Opening Book Move Count', () => {
...
GM_registerMenuCommand('[ACAS] Set Opening Book Arrow Color', () => {
    const input = prompt('Enter arrow color as hex (e.g. #f0a500 = gold, #00bfff = blue, #ff4444 = red):', openingBookPrimaryColor);
    if(!input) return;
    ...
    GM_notification({ title: 'ACAS Opening Book', text: `Arrow color set to ${hex}`, timeout: 2000 });
});
    ...
    tooltip.innerHTML = `
        <div style="font-size:11px;color:#888;margin-bottom:6px;text-transform:uppercase;letter-spacing:0.5px;">GM Book — Top Moves</div>
        ${rows}
    `;
    tooltip.style.cssText = `
        position: fixed;
...
function lookupAndDrawOpeningBookArrow(fen) {
    ...
    // Detect transition from in-book to out-of-book — flash red indicator
    if(wasInBook && !nowInBook) {
        showOutOfBookIndicator();
    }
    wasInBook = nowInBook;
    ...
    // Arrow thickness scales with popularity (weight ratio vs best move)
    const maxWeight = toShow[0].weight;
    ...
    toShow.forEach((move, rank) => {
        ...
        // Opacity fades for lower ranked moves
        const opacity = Math.max(0.35, 0.88 - rank * (0.45 / Math.max(toShow.length - 1, 1)));

        // Line width scales with weight ratio — more popular = thicker arrow
        const weightRatio = move.weight / maxWeight;
        const lineWidth = Math.round(12 + weightRatio * 16); // 12–28px range
        const arrowheadWidth = Math.round(40 + weightRatio * 32); // 40–72px
        const arrowheadHeight = Math.round(30 + weightRatio * 24); // 30–54px
