...
let openingBookArrowElems = []; // array — one per book move shown
...
function lookupAndDrawOpeningBookArrow(fen) {
    ...
    if(!moves || moves.length === 0) return; // out of book — no arrows
    ...
    toShow.forEach((move, rank) => {
        ...
        // Best move = full opacity, each lower rank fades slightly
        ...
        const elem = BoardDrawer.createShape('arrow', [from, to], {
            ...
            lineWidth: rank === 0 ? 26 : 18,
            arrowheadWidth: rank === 0 ? 70 : 52,
            arrowheadHeight: rank === 0 ? 52 : 38,
            startOffset: 26
        });
...
GM_registerMenuCommand('[ACAS] Toggle Opening Book Arrows', () => {
    ...
    if(!openingBookEnabled) clearOpeningBookArrow();
    GM_notification({
