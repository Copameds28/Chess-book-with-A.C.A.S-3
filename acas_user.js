...
// ─────────────────────────────────────────────────────────────────────────────
// OPENING BOOK
// JSON opening book keyed by "position side-to-move" (first 2 FEN fields).
// When the current position is in the book, gold arrows are drawn for the
// top book moves. When the position leaves the book the arrows disappear.
//
// ── JSON FORMAT ──────────────────────────────────────────────────────────────
// Each position key maps to an array of move objects:
//   { "uci": "e2e4", "weight": 100, "name": "King's Pawn Opening" }
// The "name" field is optional. If present it appears on the board arrow label
// and in the side panel. You can add multiple JSON files; they are all fetched
// in parallel and merged automatically.
// ─────────────────────────────────────────────────────────────────────────────

// ── ADD YOUR BOOK FILES HERE ──────────────────────────────────────────────────
// Each URL should be a raw JSON file in the same format as gm_openings.json.
...
// Fallback built-in mini-book used when all remote files fail to load.
const BUILTIN_OPENING_BOOK = {
    "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w": [
        { uci: "e2e4", weight: 100, name: "King's Pawn" },
        { uci: "d2d4", weight: 90,  name: "Queen's Pawn" },
        { uci: "c2c4", weight: 70,  name: "English Opening" },
        { uci: "g1f3", weight: 60,  name: "Réti Opening" }
    ],
    "rnbqkbnr/pppppppp/8/8/4P3/8/PPPP1PPP/RNBQKBNR b": [
        { uci: "e7e5", weight: 100, name: "Open Game" },
        { uci: "c7c5", weight: 95,  name: "Sicilian Defence" },
        { uci: "e7e6", weight: 80,  name: "French Defence" },
        { uci: "c7c6", weight: 75,  name: "Caro-Kann" }
    ],
    "rnbqkbnr/pppppppp/8/8/3P4/8/PPP1PPPP/RNBQKBNR b": [
        { uci: "d7d5", weight: 100, name: "Closed Game" },
        { uci: "g8f6", weight: 90,  name: "Indian Defence" },
        { uci: "e7e6", weight: 80,  name: "French-Indian" },
        { uci: "c7c5", weight: 75,  name: "Benoni" }
    ],
    "rnbqkbnr/pppp1ppp/8/4p3/4P3/8/PPPP1PPP/RNBQKBNR w": [
        { uci: "g1f3", weight: 100, name: "King's Knight" },
        { uci: "f1c4", weight: 80,  name: "Italian/Bishop" },
        { uci: "f2f4",  weight: 60, name: "King's Gambit" },
        { uci: "d2d4",  weight: 55, name: "Center Game" }
    ],
    "rnbqkbnr/pp1ppppp/8/2p5/4P3/8/PPPP1PPP/RNBQKBNR w": [
        { uci: "g1f3", weight: 100, name: "Open Sicilian" },
        { uci: "c2c3", weight: 70,  name: "Alapin" },
        { uci: "f1c4", weight: 60,  name: "Sicilian Bc4" },
        { uci: "b1c3", weight: 55,  name: "Closed Sicilian" }
    ],
    "rnbqkbnr/pppppppp/8/8/2P5/8/PP1PPPPP/RNBQKBNR b": [
        { uci: "c7c5", weight: 100, name: "Symmetrical English" },
        { uci: "e7e5", weight: 90,  name: "King's English" },
        { uci: "g8f6", weight: 75,  name: "English / Réti" },
        { uci: "e7e6", weight: 70,  name: "English / French" }
    ],
    "rnbqkb1r/pppppppp/5n2/8/3P4/8/PPP1PPPP/RNBQKBNR w": [
        { uci: "c2c4", weight: 100, name: "Indian Systems" },
        { uci: "g1f3", weight: 90,  name: "Torre / London" },
        { uci: "e2e3", weight: 60,  name: "Colle System" }
    ],
    "rnbqkbnr/ppp1pppp/8/3p4/3P4/8/PPP1PPPP/RNBQKBNR w": [
        { uci: "c2c4", weight: 100, name: "Queen's Gambit" },
        { uci: "g1f3", weight: 90,  name: "London / Torre" },
        { uci: "b1c3", weight: 70,  name: "Veresov" },
        { uci: "e2e3", weight: 50,  name: "Colle System" }
    ]
};

let openingBook         = null;
let openingBookArrowElems = [];
let openingBookLabelElems = [];   // board square labels showing opening name
let openingBookEnabled  = true;
let openingBookShowLabels = GM_getValue('openingBookShowLabels', true);
let openingBookMaxMoves = parseInt(GM_getValue('openingBookMaxMoves', '3'));
let openingBookPrimaryColor = GM_getValue('openingBookPrimaryColor', '#f0a500');
let openingBookTooltipPos   = JSON.parse(GM_getValue('openingBookTooltipPos', 'null'));

// ── LOAD (multiple files, merged) ────────────────────────────────────────────
async function loadOpeningBook() {
    const results = await Promise.allSettled(
        OPENING_BOOK_URLS.map(url =>
    ...
    for (const result of results) {
        ...
        for (const [key, moves] of Object.entries(book)) {
            ...
            if (!merged[key]) {
                merged[key] = moves.slice();
            } else {
                // Append moves not already present (dedup by UCI)
                const seen = new Set(merged[key].map(m => m.uci));
                for (const m of moves) {
                    if (!seen.has(m.uci)) { merged[key].push(m); seen.add(m.uci); }
                }
            }
        }
    }
    ...
    if (loadedCount > 0) {
        openingBook = merged;
        if (debugModeActivated) console.log('[ACAS Opening Book] Loaded', loadedCount, 'file(s),', Object.keys(merged).length, 'positions');
    } else {
        if (debugModeActivated) console.warn('[ACAS Opening Book] All remote files failed, using built-in mini-book');
        openingBook = BUILTIN_OPENING_BOOK;
    }
}

// ── CLEAR ─────────────────────────────────────────────────────────────────────
function clearOpeningBookArrow() {
    for (const elem of openingBookArrowElems) { try { elem.remove(); } catch (_) {} }
    openingBookArrowElems = [];
    for (const elem of openingBookLabelElems)  { try { elem.remove(); } catch (_) {} }
    openingBookLabelElems = [];
}

// ── BOARD LABEL (floating div at destination square) ─────────────────────────
const RANK_COLORS = ['#f0a500', '#aaaaaa', '#cd7f32', '#88bbff', '#ff9988'];

function createBoardLabel(toSquare, text, color) {
    if (!text || !chessBoardElem) return null;
    try {
        const coords = fenCoordArrToDomCoord([toSquare]);
...
// ── SIDE-PANEL TOOLTIP ────────────────────────────────────────────────────────
function showWinRateTooltip(moves, totalWeight) {
    const existing = document.getElementById('acas-winrate-tooltip');
    if (existing) existing.remove();
    if (!moves || moves.length === 0) return;

    const boardElem = getBoardElem();
    const rect = boardElem ? boardElem.getBoundingClientRect() : null;
    const defaultLeft = rect ? Math.round(rect.right + 10) : 20;
    const defaultTop  = rect ? Math.round(rect.top) : 100;
    const pos = openingBookTooltipPos || { left: defaultLeft, top: defaultTop };

    const topMoves = [...moves].sort((a, b) => b.weight - a.weight).slice(0, Math.min(3, moves.length));

    const rows = topMoves.map((m, i) => {
        const pct      = totalWeight > 0 ? ((m.weight / totalWeight) * 100).toFixed(1) : '0.0';
        const barWidth = Math.round((m.weight / topMoves[0].weight) * 100);
        const color    = RANK_COLORS[i] || '#888888';
        const nameHtml = m.name
            ? '<div style="font-size:10px;color:#bbb;margin-top:1px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">' + m.name + '</div>'
            : '';
        return '<div style="margin-bottom:8px">' +
            '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:2px">' +
            '<span style="font-family:monospace;font-size:13px;font-weight:bold;color:' + color + '">' + m.uci + '</span>' +
            '<span style="font-size:11px;color:#aaa">' + pct + '%</span>' +
            '</div>' +
            nameHtml +
            '<div style="background:#444;border-radius:3px;height:4px;margin-top:4px">' +
            '<div style="background:' + color + ';height:4px;border-radius:3px;width:' + barWidth + '%"></div>' +
            '</div></div>';
    }).join('');

    const tooltip = document.createElement('div');
    tooltip.id = 'acas-winrate-tooltip';
    tooltip.innerHTML =
        '<div style="font-size:10px;color:#666;margin-bottom:8px;text-transform:uppercase;' +
        'letter-spacing:0.6px;cursor:move;user-select:none" id="acas-winrate-handle">&#9776; GM Book moves</div>' +
        rows;
    ...
    const handle = document.getElementById('acas-winrate-handle');
    if (handle) {
        let dragging = false, startX, startY, origLeft, origTop;
        handle.addEventListener('mousedown', function(e) {
            dragging = true; startX = e.clientX; startY = e.clientY;
            origLeft = parseInt(tooltip.style.left); origTop = parseInt(tooltip.style.top);
            e.preventDefault();
        });
        ...
        document.addEventListener('mouseup', function() {
            if (!dragging) return;
            dragging = false;
            openingBookTooltipPos = { left: parseInt(tooltip.style.left), top: parseInt(tooltip.style.top) };
            GM_setValue('openingBookTooltipPos', JSON.stringify(openingBookTooltipPos));
        });
    }
}

function hideWinRateTooltip() {
    const existing = document.getElementById('acas-winrate-tooltip');
    if (existing) existing.remove();
}

// ── MAIN DRAW FUNCTION ────────────────────────────────────────────────────────
function lookupAndDrawOpeningBookArrow(fen) {
    clearOpeningBookArrow();
    if (!openingBookEnabled || !openingBook || !BoardDrawer || !fen) return;

    const fenKey = fen.split(' ').slice(0, 2).join(' ');
    const moves  = openingBook[fenKey];
    if (!moves || moves.length === 0) { hideWinRateTooltip(); return; }

    const totalWeight = moves.reduce(function(s, m) { return s + m.weight; }, 0);
    const sorted  = moves.slice().sort(function(a, b) { return b.weight - a.weight; });
    const toShow  = sorted.slice(0, openingBookMaxMoves);
    const maxWeight = toShow[0].weight;

    toShow.forEach(function(move, rank) {
        const from  = move.uci.slice(0, 2);
        const to    = move.uci.slice(2, 4);
        const color = RANK_COLORS[rank] || RANK_COLORS[RANK_COLORS.length - 1];

        // Arrow opacity fades for lower-ranked moves
        const opacity     = Math.max(0.35, 0.88 - rank * (0.45 / Math.max(toShow.length - 1, 1)));
        const ratio       = move.weight / maxWeight;
        const lineWidth       = Math.round(12 + ratio * 16);
        const arrowheadWidth  = Math.round(40 + ratio * 32);
        const arrowheadHeight = Math.round(30 + ratio * 24);

        const arrowColor = rank === 0 ? openingBookPrimaryColor : color;

        const arrowElem = BoardDrawer.createShape('arrow', [from, to], {
            style: 'fill:' + arrowColor + ';opacity:' + opacity.toFixed(2) +
                   ';stroke:rgba(0,0,0,0.45);stroke-width:2px;stroke-linejoin:round;',
            lineWidth, arrowheadWidth, arrowheadHeight, startOffset: 26
        });
        if (arrowElem) openingBookArrowElems.push(arrowElem);

        // Board label at destination square (if move has a name)
        if (openingBookShowLabels && move.name) {
            const labelElem = createBoardLabel(to, move.name, arrowColor);
            if (labelElem) openingBookLabelElems.push(labelElem);
        }
    });

    showWinRateTooltip(sorted, totalWeight);
}


GM_registerMenuCommand('[ACAS] Toggle Opening Book', function() {
    openingBookEnabled = !openingBookEnabled;
    if (!openingBookEnabled) { clearOpeningBookArrow(); hideWinRateTooltip(); }
    GM_notification({ title: 'ACAS Opening Book',
        text: 'Opening book ' + (openingBookEnabled ? 'enabled \u2713' : 'disabled \u2717'),
        timeout: 2500 });
});

GM_registerMenuCommand('[ACAS] Toggle Opening Name Labels', function() {
    openingBookShowLabels = !openingBookShowLabels;
    GM_setValue('openingBookShowLabels', openingBookShowLabels);
    GM_notification({ title: 'ACAS Opening Book',
        text: 'Board name labels ' + (openingBookShowLabels ? 'ON \u2713' : 'OFF \u2717'),
        timeout: 2000 });
});

GM_registerMenuCommand('[ACAS] Opening Book Move Count', function() {
    const input = prompt('How many opening book moves to show? (1-5)', String(openingBookMaxMoves));
    if (input === null) return;
    const n = Math.min(5, Math.max(1, parseInt(input) || 1));
    openingBookMaxMoves = n;
    GM_setValue('openingBookMaxMoves', String(n));
    GM_notification({ title: 'ACAS Opening Book', text: 'Showing top ' + n + ' book move(s)', timeout: 2000 });
});

GM_registerMenuCommand('[ACAS] Opening Book Primary Arrow Color', function() {
    const input = prompt('Primary arrow color as hex (#f0a500=gold, #00bfff=blue, #ff4444=red):', openingBookPrimaryColor);
    if (!input) return;
    const hex = input.trim();
    if (!/^#[0-9a-fA-F]{6}$/.test(hex)) {
        GM_notification({ title: 'ACAS Opening Book', text: 'Invalid color — use 6-digit hex like #f0a500', timeout: 2500 });
        return;
    }
    openingBookPrimaryColor = hex;
    GM_setValue('openingBookPrimaryColor', hex);
    GM_notification({ title: 'ACAS Opening Book', text: 'Primary arrow color set to ' + hex, timeout: 2000 });
});

GM_registerMenuCommand('[ACAS] Reload Opening Book', function() {
    openingBook = null;
    loadOpeningBook().then(function() {
        const count = openingBook ? Object.keys(openingBook).length : 0;
        GM_notification({ title: 'ACAS Opening Book', text: 'Book reloaded — ' + count + ' positions', timeout: 2500 });
    });
});
...
async function processBoardPosition(currentFullFen = getFen(), squareChangeAmount = 0) {
    ...
    // Draw opening book arrow (gold) if position is in book; clears itself when out of book
    lookupAndDrawOpeningBookArrow(currentFullFen);
}
...
async function start() {
    ...
    // Load opening book once (async, non-blocking)
    if(!openingBook) loadOpeningBook();
