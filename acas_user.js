...
let openingBookOutIndicatorElem = null; // red flash element when going out of book
let wasInBook = false; // track previous position book status
...
// Show a brief red overlay flash on the board when going out of book
function showOutOfBookIndicator() {
    clearOutOfBookIndicator();
    // Find the board element to overlay
    const board = document.querySelector('chess-board, cg-board, .board, #board') ||
    ...
    setTimeout(() => { div.style.opacity = '0'; }, 600);
    setTimeout(() => { clearOutOfBookIndicator(); }, 1300);
}

// Show win rate tooltip near the board
function showWinRateTooltip(moves, totalWeight) {
    ...
    const rows = topMoves.map((m, i) => {
        ...
        const barWidth = totalWeight > 0 ? Math.round((m.weight / topMoves[0].weight) * 100) : 0;
        const rankColors = ['#f0a500', '#aaaaaa', '#cd7f32'];
        const color = rankColors[i] || '#888';
        return `
            <div style="margin-bottom:5px;">
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:2px;">
                    <span style="font-family:monospace;font-size:13px;font-weight:bold;color:${color};">${m.uci}</span>
                    <span style="font-size:12px;color:#ccc;">${pct}% of GM games</span>
                </div>
                <div style="background:#333;border-radius:3px;height:6px;width:100%;">
                    <div style="background:${color};height:6px;border-radius:3px;width:${barWidth}%;"></div>
                </div>
            </div>`;
    }).join('');
