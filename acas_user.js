...
function showWinRateTooltip(moves, totalWeight) {
    ...
    tooltip.innerHTML = `<div style="font-size:10px;color:#888;margin-bottom:7px;text-transform:uppercase;letter-spacing:0.6px">GM Book moves</div>${rows}`;
...
function lookupAndDrawOpeningBookArrow(fen) {
    ...
    if(wasInBook && !nowInBook) showOutOfBookIndicator();
    wasInBook = nowInBook;
    if(!nowInBook) { hideWinRateTooltip(); return; }
